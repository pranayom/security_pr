"""Scorecard formatting — JSON output and Rich terminal rendering."""
...
from src.gatekeeper.models import AssessmentScorecard, FlagSeverity, Verdict
...
def scorecard_to_json(scorecard: AssessmentScorecard) -> str:
    """Serialize scorecard to JSON."""
    return scorecard.model_dump_json(indent=2)


def render_scorecard(scorecard: AssessmentScorecard, console: Console | None = None) -> None:
    """Render a Rich-formatted scorecard to the console."""
    if console is None:
        console = Console()
    ...
    header = (
        f"[{style}]{label}[/{style}]\n\n"
        f"PR: {scorecard.owner}/{scorecard.repo}#{scorecard.pr_number}\n"
        f"Confidence: {scorecard.confidence:.0%}\n\n"
        f"{scorecard.summary}"
    )
    console.print(Panel(header, title="Assessment Scorecard", border_style=style))
